...
def norm_dist_version( version ):
  ...
  if not pep440_version.fullmatch( version ):
    raise PEPValidationError(
      pep = 440,
      msg = "public version identifiers MUST comply with the following scheme, [N!]N(.N)*[{a|b|rc}N][.postN][.devN]",
      val = version )
...
def norm_dist_author( name, email ):
  ...
  #.............................................................................
  # > If both email and name are provided, the value goes in Author-email/Maintainer-email
...
def norm_dist_build( build ):
  ...
  if not pep427_build.fullmatch( build ):
    raise PEPValidationError(
      pep = 427,
      msg = "Must start with a digit. Acts as a tie-breaker if two wheel file names are the same in all other respects",
      val = build )
...
def norm_dist_compat( py_tag, abi_tag, plat_tag ):
  ...
  # > platform tag is simply distutils.util.get_platform() with all hyphens - and periods . replaced with underscore _.
  plat_tag = re.sub( r'[\-\_\.]+', "_", norm_printable(plat_tag).lower() )

  if not pep425_pytag.fullmatch( py_tag ):
    raise PEPValidationError(
      pep = 425,
      msg = "The version is py_version_nodot. CPython gets away with no dot, but if one is needed the underscore _ is used instead",
      val = py_tag )
  ...
  if not pep425_pytag.fullmatch( plat_tag ):
    ...
    raise PEPValidationError(
      pep = 425,
      msg = "platform tag is simply distutils.util.get_platform() with all hyphens - and periods . replaced with underscore _",
      val = plat_tag )
...
def norm_wheel_name( name ):
  ...
  # > Each component of the filename is escaped by replacing runs of non-alphanumeric
...
def norm_mode( mode = None ):
  ...
  if mode & stat.S_IXUSR:
    # if the mode is executable by the user, set as executable also by group and others
    _mode |= 0o111
...
def mode_to_xattr( mode = None ):
  ...
  The file permission mode is masked and shifted to the second most significant byte.
  ...
  # MS-DOS directory flag 0x10 (various sources, e.g. https://stackoverflow.com/questions/434641 )
...
# NOTE: email names are notoriously hard to validate correctly, this is probably not correct.
...
# ensures the name does not include double-quotes, backslashes, or linefeeds or tabs
# the '@' here is also included here
pep621_author_name = re.compile( r'^([^\"\\\,\@\r\n\t\f\v]+)?$', re.UNICODE )

# ensures that there is a single `@` separating two non-empty segments, and each segment
# does not contain white-space or another `@`
...
common_plattag = {
  ...
  'linux' : re.compile( r'^((many)?linux(_(\d\d?)_(\d\d?))?_(i386|x86_64|i686|aarch64|armv7l|ppc64|ppc64le|s390x))$', re.IGNORECASE ),
  'local' : re.compile( rf'^({local_plat})$', re.IGNORECASE )}

#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# https://www.python.org/dev/peps/pep-0301/#distutils-trove-classification
# > It was decided that strings would be used for the classification entries
# > due to the deep nesting that would be involved in a more formal Python structure.
...
# This allows any package or version text, brackets, parentheses, spaces, and forward slash.
# TODO: write test against current list of classifiers
pep_301_classifier = re.compile( r'^[A-Z0-9._\-\/\[\]\(\) ]+( :: [A-Z0-9._\-\/\[\]\(\) ]+)*$', re.IGNORECASE )
